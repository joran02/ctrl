...
/*    log_debug(ZONE,"creating new hash table of size %d",prime); */
    ...
    xnew = (xht)malloc(sizeof(_xht)*1);
    memset(xnew,0,sizeof(_xht)*1);
    xnew->prime = prime;
    xnew->zen = (struct xhn_struct *)malloc( sizeof(_xhn)*prime); /* array of xhn size of prime */
    
    memset(xnew->zen,0,sizeof(_xhn)*prime);
    xnew->free_list = NULL;
...
/*        log_debug(ZONE,"replacing %s with new val %X",key,val); */
        ...
        if(n->keylen < len ){//重新分配地址
            
            n->key = (char *)realloc(n->key, len+1);
        }
        strcpy(n->key, key);
        //n->key = key;
        n->keylen = len;
